...
% =========================================================
% AP (spike) 路径
base_path = 'F:\YlabPTZnxpxlrecording\2025\2025-04-25_983_WT-male-adult\2025-04-25_15-48-05\Record Node 101\experiment1\recording2\continuous\Neuropix-PXI-100.ProbeA-AP';
...
% =========================================================
win = [650 740];% WT1347PTZ
%win = [1720 1810]; %  WT983baseline      可视时间窗口 [s]
x_start = win(1);
x_end   = win(2);
...
%% (3) Raster: 绝对 depth
% ============================================================
% === 统一模板ID → 深度(µm) 映射（自动判基 + 越界保护） ===
% 依赖：ephysData, qMetric
% ============================================================

% 原始模板ID，可能是0-based
spike_templates0 = double(ephysData.spike_templates(:));
% 模板总数（来自 ephysData.templates）
if isfield(ephysData,'templates') && ~isempty(ephysData.templates)
    nTemplatesT = size(ephysData.templates,1);
else
    nTemplatesT = 0;
end
% qMetric.maxChannels 长度
if isfield(qMetric,'maxChannels') && ~isempty(qMetric.maxChannels)
    nQ = numel(qMetric.maxChannels);
else
    nQ = 0;
end

% ---- 自动判断是0-based还是1-based ----
if nTemplatesT > 0 && min(spike_templates0)==0 && max(spike_templates0) <= nTemplatesT-1
    tmpl1 = spike_templates0 + 1;      % 0-based → 1-based
elseif nTemplatesT > 0 && min(spike_templates0)>=1 && max(spike_templates0) <= nTemplatesT
    tmpl1 = spike_templates0;          % 已经是1-based
else
    tmpl1 = spike_templates0;          % 混合或未知，后续按域过滤
    warning('spike_templates 与 templates 数量不匹配，将过滤越界模板。');
end

% ---- 过滤越界模板ID（相对于 templates 与 qMetric 的共同最小长度）----
maxDomain = max([nTemplatesT, nQ]);    % 用两者中较大者兜底（下方再逐步校验）
if maxDomain==0
    error('templates 与 qMetric.maxChannels 至少需要有一个可用以建立深度映射。');
end
limitForFilter = min(max([1,nTemplatesT]), max([1,nQ])); % 共同可用域的上界（>=1）
if limitForFilter < 1
    error('无法建立模板ID有效域，请检查 ephysData.templates 和 qMetric.maxChannels。');
end

validMask_st = (tmpl1 >= 1) & (tmpl1 <= limitForFilter);
nBad = sum(~validMask_st);
if nBad > 0
    badIDs = unique(tmpl1(~validMask_st));
    warning('检测到 %d 个越界 spike（模板ID: %s），这些点将被跳过。', ...
        nBad, mat2str(badIDs(:)'));
end

% ---- 有效模板集合（1-based，且在共同有效域内）----
uniqueTemps = unique(tmpl1(validMask_st), 'stable');

% ---- 通道Y坐标（µm）----
chanY = ephysData.channel_positions(:,2);   % [nChan x 1]
nChan = numel(chanY);

% ---- 为每个模板决定最大通道：优先 qMetric，缺失则从 templates 计算 ----
maxTID = max(uniqueTemps);
maxCh_of_temp = nan(maxTID, 1);

% 1) qMetric.maxChannels（若存在且足够长）
if isfield(qMetric,'maxChannels') && ~isempty(qMetric.maxChannels)
    mcAll = qMetric.maxChannels(:);
    useIDs = uniqueTemps(uniqueTemps <= numel(mcAll));
    maxCh_of_temp(useIDs) = mcAll(useIDs);
end

% 2) 对仍为NaN者，且 ephysData.templates 有对应模板时，从波形振幅计算
if isfield(ephysData,'templates') && ~isempty(ephysData.templates)
    nTemplatesT = size(ephysData.templates,1);
    for tID = uniqueTemps(:)'
        if isnan(maxCh_of_temp(tID)) && tID <= nTemplatesT
            chAmp = squeeze(max(ephysData.templates(tID,:,:),[],2) - ...
                            min(ephysData.templates(tID,:,:),[],2)); % [1 x nChan]
            [~, mx] = max(chAmp,[],2);
            if ~isempty(mx) && mx >= 1 && mx <= nChan
                maxCh_of_temp(tID) = mx;
            end
        end
    end
end
...
% 3) 模板ID → 深度(µm)
temp2depth_um = nan(maxTID,1);
okIDs = uniqueTemps(~isnan(maxCh_of_temp(uniqueTemps)) & ...
                    maxCh_of_temp(uniqueTemps)>=1 & ...
                    maxCh_of_temp(uniqueTemps)<=nChan);
temp2depth_um(okIDs) = chanY(maxCh_of_temp(okIDs));

% ---- 诊断输出 ----
fprintf('[diag] max(uniqueTemps)=%d, numel(qMetric.maxChannels)=%d, size(templates,1)=%d\n', ...
    max(uniqueTemps), ...
    (isfield(qMetric,'maxChannels') && ~isempty(qMetric.maxChannels)) * numel(qMetric.maxChannels), ...
    (isfield(ephysData,'templates') && ~isempty(ephysData.templates)) * size(ephysData.templates,1));

% ============================================================
% === 保险1：规范 template_id_per_row 为 1-based ===
% 依赖：template_id_per_row, ephysData.templates
% ============================================================
template_id_per_row = template_id_per_row(:);   % 列向量
if ~isempty(template_id_per_row)
    % 如果看起来是0-based，且与 templates 数量相容，则 +1
    if isfield(ephysData,'templates') && ~isempty(ephysData.templates)
        nTemplatesT = size(ephysData.templates,1);
        if min(template_id_per_row)==0 && max(template_id_per_row) <= nTemplatesT-1
            template_id_per_row = template_id_per_row + 1;
        end
    else
        % 没有 templates 的情况下，若看起来是0-based且非负，也做 +1（保守处理）
        if min(template_id_per_row)==0
            template_id_per_row = template_id_per_row + 1;
        end
    end
end
...
%% ============================================================
% === Raster 绘制（使用统一深度映射，与 probe 散点图对齐） ===
% 依赖：tl2, resultArray_type12, template_id_per_row, unitType, win
% ============================================================
...
for i = 1:size(resultArray_type12,1)
    if i > numel(template_id_per_row) || isnan(template_id_per_row(i))
        rowsSkipped = rowsSkipped + 1;
        continue;
    end
    template_id = template_id_per_row(i);   % 期望1-based

    % ---- 用统一映射取深度 ----
    if template_id >= 1 && template_id <= numel(temp2depth_um)
        this_depth = temp2depth_um(template_id);
    else
        this_depth = NaN;
    end
    if isnan(this_depth)
    ...
    % ---- unitType 决定颜色（1=红，2=蓝；其他跳过或统一颜色）----
    if template_id <= numel(unitType) && unitType(template_id) == 1
        c = 'r';
    else
        c = 'b';
    end
...
% === 统一坐标轴、格式 ===
xlim(win);
ymin = min(chanY); ymax = max(chanY);
ylim([ymin ymax]);                  % 与 probe 图一致的 y 轴范围
set(gca,'YDir','normal');           % 深部在更大 y
xlabel('Time (s)');
ylabel('Depth (\mum)');
title('Raster (unitType 1=red, 2=blue) — absolute depth');
grid on; hold off;

% === 保险2：统计被跳过的行（无效映射/越界等） ===
bad_rows = false(size(template_id_per_row));
for i = 1:numel(template_id_per_row)
    tid = template_id_per_row(i);
    if isnan(tid) || tid < 1 || tid > numel(temp2depth_um)
        bad_rows(i) = true;
    else
        if isnan(temp2depth_um(tid))
            bad_rows(i) = true;
        end
    end
end
fprintf('[diag] rows skipped in raster due to invalid depth mapping: %d / %d\n', ...
        sum(bad_rows), numel(template_id_per_row));
...
% ------- 11.2 保存路径设置 -------
save_dir = 'C:\Users\lab-admin\OneDrive - purdue.edu\1.final_paper\final_figures\DataAnalysis\Fig1_ephys_WT_HOM\WT_983_PTZ';
