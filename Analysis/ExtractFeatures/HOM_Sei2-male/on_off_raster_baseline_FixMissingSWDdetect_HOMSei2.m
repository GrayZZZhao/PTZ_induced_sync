...
spike_time_wt = readNPY('H:\seizure\2024-01-02_WT_HOM-male-adult\2024-01-02_13-43-59\Record Node 101\experiment2\recording2\continuous\Neuropix-PXI-100.ProbeA-AP\spike_times.npy');
spike_time_full_wt = double(spike_time_wt) / 30000;
spike_templates_wt = readNPY('H:\seizure\2024-01-02_WT_HOM-male-adult\2024-01-02_13-43-59\Record Node 101\experiment2\recording2\continuous\Neuropix-PXI-100.ProbeA-AP\spike_templates.npy');
...
% ==== Enforce continuity after onset change (label==2 & keep==true) ====

% 依赖
req = {'swd_events_filtered','swd_event_quality','swd_keep_idx','swd_session_file_mat_merged','fs'};
for ii = 1:numel(req)
    if ~evalin('base', sprintf('exist(''%s'',''var'')', req{ii}))
        error('Missing "%s". Please run the previous steps first.', req{ii});
    end
end

swd_events_filtered = evalin('base','swd_events_filtered');   % cell of kept events
EQ        = evalin('base','swd_event_quality');               % Mx4 [event_id, t_start_s, t_end_s, label]
keep_idx  = evalin('base','swd_keep_idx');                    % Mx1 logical (kept in curate)
matMerged = evalin('base','swd_session_file_mat_merged');     % ..._onsetMerged.mat
fs        = evalin('base','fs');

% 载入 onsetMerged（优先 table）
S = load(matMerged);
if isfield(S,'T_merged')
    event_id_vec  = S.T_merged.event_id;
    t_start_s_new = S.T_merged.t_start_s;
    label_final   = S.T_merged.label;
    keep_final    = logical(S.T_merged.keep);
else
    event_id_vec  = S.event_id;
    t_start_s_new = S.t_start_s;
    label_final   = S.label;
    keep_final    = logical(S.keep);
end

M = size(EQ,1);

% 原事件ID -> 合并后起点秒
tstart_by_id = nan(M,1);
tstart_by_id(event_id_vec) = t_start_s_new;

% 目标集合：label==2 & keep==true（以合并后的为准，缺失则退回EQ/keep_idx）
if numel(label_final)==M && numel(keep_final)==M
    mask_target = (label_final==2) & keep_final;
else
    mask_target = (EQ(:,4)==2) & keep_idx(:);
end

% 原事件ID（保持顺序） -> swd_events_filtered 中的位置
kept_ids = find(keep_idx(:));
pos_map  = zeros(M,1);
pos_map(kept_ids) = 1:numel(kept_ids);

% 备份
assignin('base','swd_events_filtered_backup', swd_events_filtered);

n_updated=0; n_clamped=0;
for j = find(mask_target(:))'   % j 是原事件ID
    p = pos_map(j);
    if p<=0 || p>numel(swd_events_filtered), continue; end

    ev = swd_events_filtered{p};
    if isempty(ev), continue; end

    t0 = tstart_by_id(j);
    if isnan(t0), continue; end

    new_start = round(t0*fs) + 1;
    ...
    % === 关键：强制连续 ===
    % 用 [new_start : ev(end)] 重建事件，确保第一项与后续“连续整数”
    ev_new = new_start : ev(end);

    % 写回
    swd_events_filtered{p} = ev_new;
    n_updated = n_updated + 1;
end

assignin('base','swd_events_filtered', swd_events_filtered);

fprintf('Continuity enforced for %d events (label==2 & keep==true).\n', n_updated);
if n_clamped>0
    fprintf('  %d new_start values were clamped within original event bounds.\n', n_clamped);
end
fprintf('Backup saved as: swd_events_filtered_backup\n');

%
% ==== Save swd_events_filtered to .mat (prevent overwrite) ====
save_dir = fileparts(matMerged);  % 存在同目录下
timestamp = datestr(now, 'yyyymmdd_HHMMSS');
save_name = fullfile(save_dir, ['swd_events_filtered_' timestamp '.mat']);

try
    save(save_name, 'swd_events_filtered');
    fprintf('Saved swd_events_filtered to: %s\n', save_name);
catch ME
    warning('Failed to save swd_events_filtered: %s', ME.message);
end
...
selected_neurons = find(burst_to_non_burst_rate_ratio >0.01);  %(<0.5 meaning off,    >2 meaing 0n )
...
% --- Plot in the first subplot (CWT 0–60 Hz) ---
% ax(1) = subplot('Position', subplot_positions(1,:)); hold on
% 
% % 基本参数（尽量复用你现有变量）
% fs = 2500;
...
% end
% 
% % 信号：与后面子图一致，这里用 mean(d)
% sig = mean(d);
% sig = sig(:);
% N   = numel(sig);
% 
% % 仅计算 [x_start, x_end] 时间窗，避免整段计算浪费
% ix0 = max(1, floor(x_start*fs)+1);
% ix1 = min(N, ceil(x_end*fs));
% sig_win = sig(ix0:ix1);
% t_win   = ((ix0:ix1)-1)/fs;  % 秒
% 
% % CWT（amor），频率锁定到 0.5–60 Hz，确保多段对齐一致
% [cfs, f] = cwt(sig_win, fs, 'amor', 'FrequencyLimits', [0.5 60]);  % cfs: [freq x time]
% 
% 
% Z = 20*log10(abs(cfs) + eps);   % dB 幅度
% 
% % 稳健色阶：5–95 分位
% clim = [prctile(Z(:),5), prctile(Z(:),95)];
% 
% % 绘图
% imagesc(t_win, f, Z);
% axis xy
...
% xlabel('Time (s)')
% ylabel('Frequency (Hz)')
% title('CWT Scalogram (amor, 0–60 Hz)')
% colormap(jet)
% caxis(clim)
% % colorbar  % 需要就打开
% 
...
% 建议把单位转为 µV（如果 d 是原始 ADC/单位）
scale_factor = 0.195;           % 每单位 -> µV（按你前面滤波那段用的）
sig = mean(d) * scale_factor;   % 转 µV
sig = sig(:);
N   = numel(sig);
...
% === 关键修改：用“功率”并取对数 ===
P = abs(cfs).^2;                % 功率 ~ µV^2
Z = log10(P + eps);             % log(power) —— 与示例配色条风格一致

% 稳健色阶
clim = [prctile(Z(:),5), prctile(Z(:),95)];

% 作图
imagesc(t_win, f, Z); axis xy
xlim([x_start, x_end]); ylim([0.5, 60])
xlabel('Time (s)'); ylabel('Frequency (Hz)')
title('CWT (amor, 0–60 Hz) — log(power)')
colormap(jet); caxis(clim)

cb = colorbar;
ylabel(cb, 'Log power (\muV^2)')   % 配色条标签
...
ax(2)=subplot('Position', subplot_positions(3, :));
