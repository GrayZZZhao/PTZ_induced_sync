...
excel_path = 'H:\seizure\2024-01-02_WT_HOM-male-adult\2024-01-02_13-43-59\Record Node 101\experiment2\recording2\continuous\Neuropix-PXI-100.ProbeA-AP\clustertwo.xlsx';
...
%%
% ===== Filter real onset times within SWD event windows =====
% Requires:
%   swd_events_seconds : 1xN cell, each cell = event time series (in seconds)
%   swd_event_quality  : Mx4 matrix, 2nd column = real onset times (s)

% ---- Extract real onsets ----
swd_real_onset = swd_event_quality(:,2);
s = swd_real_onset(:);

% ---- Filter by event windows ----
keep = false(size(s));
for k = 1:numel(swd_events_seconds)
    win = swd_events_seconds{k};
    if isempty(win) || all(isnan(win)), continue; end
    a = min(win);   % window start (s)
    b = max(win);   % window end (s)
    keep = keep | (s >= a & s <= b);  % keep if inside any window
end
...
% ---- Create save folder (avoid overwrite) ----
out_dir = fullfile(pwd, 'Str_Cor_histPeak_diff');
if ~exist(out_dir, 'dir')
    mkdir(out_dir);
end
