...
spike_time_wt = readNPY('H:\npxlkv11_summer_2025\1800_HOM_M_kv11\2025-07-18_13-33-14\Record Node 101\experiment2\recording2\continuous\Neuropix-PXI-100.ProbeA-AP\spike_times.npy');
spike_time_full_wt = double(spike_time_wt) / 30000;
spike_templates_wt = readNPY('H:\npxlkv11_summer_2025\1800_HOM_M_kv11\2025-07-18_13-33-14\Record Node 101\experiment2\recording2\continuous\Neuropix-PXI-100.ProbeA-AP\spike_templates.npy');
...
selected_neurons = find(burst_to_non_burst_rate_ratio >0.1);  %(<0.5 meaning off,    >2 meaing 0n )
...
%   Keyboard: A/← previous; D/→ next; G go to; S save; Enter save & exit; Esc/Q exit
    ...
    % ================== Internals ==================
    function drawEvent()
        % guard: figure/axes may be deleted by user
        if ~isAlive(S.fig) || ~isAlive(S.ax1), return; end
        ...
        % Link X axes (only alive axes)
        axs = [S.ax0 S.ax1 S.ax2 S.ax3];
        axs = axs(arrayfun(@isAlive, axs));
        if numel(axs) >= 2
            try, linkaxes(axs,'x'); catch, end
        end
    ...
    function closeAndSave(), autosave('done'); onClose(); end
    function onClose(~,~)
        % stop timers / save once more
        try, autosave('close'); catch, end

        % unbind callbacks to avoid DestroyedObject Callback
        if isAlive(S.fig)
            try
        ...
        % clear handles to avoid later access
        try
            if isAlive(S.txt), S.txt = []; end
            if isAlive(S.ax0), S.ax0 = []; end
            if isAlive(S.ax1), S.ax1 = []; end
            if isAlive(S.ax2), S.ax2 = []; end
            if isAlive(S.ax3), S.ax3 = []; end
        catch
        end
...
function tf = isAlive(h)
    tf = ~isempty(h) && ishghandle(h) && isvalid(h);
end
