...
function unitQualityGuiHandle = BC_unitQualityGUI_GZ_v3(memMapData, ephysData, qMetric, param, probeLocation, unitType, plotRaw)
...
%% ?? amplitude axes/title/legend/spikeFR ????? axes
if ~isfield(guiData,'ampliAx') || ~isgraphics(guiData.ampliAx) || ...
        ~strcmp(get(guiData.ampliAx,'Type'),'axes')
    guiData.ampliAx = subplot(6,13,[67:70,73:76]);
    hold(guiData.ampliAx,'on');
    axes(guiData.ampliAx);
    yyaxis left;
    xlabel('Experiment time (s)');
    ylabel('Template amplitude scaling');
    yyaxis right;
    guiData.spikeFR = stairs(NaN, NaN, 'LineWidth', 2.0, 'Color', [1 0.5 0]);
    ylabel('Firing rate (sp/sec)');
    guiData.ampliTitle  = title('');
    guiData.ampliLegend = legend('','');
else
    axes(guiData.ampliAx);
end
if ~isfield(guiData,'ampliTitle') || ~isgraphics(guiData.ampliTitle)
    axes(guiData.ampliAx); yyaxis left;
    guiData.ampliTitle = title('');
end
if ~isfield(guiData,'ampliLegend') || ~isgraphics(guiData.ampliLegend)
    axes(guiData.ampliAx);
    guiData.ampliLegend = legend('','');
end
if ~isfield(guiData,'spikeFR') || ~isgraphics(guiData.spikeFR)
    axes(guiData.ampliAx); yyaxis right;
    guiData.spikeFR = stairs(NaN, NaN, 'LineWidth', 2.0, 'Color', [1 0.5 0]);
end

%% ?? raster axes/line ??
if ~isfield(guiData,'rasterAx') || ~isgraphics(guiData.rasterAx) || ...
        ~strcmp(get(guiData.rasterAx,'Type'),'axes')
    guiData.rasterAx = subplot(6,13,[71 72]);
    hold(guiData.rasterAx,'on');
    xlabel(guiData.rasterAx,'Time (s)');
    ylabel(guiData.rasterAx,'Trials');
    title(guiData.rasterAx,'Unit raster');
    ylim(guiData.rasterAx,[0.5 1.5]);
    set(guiData.rasterAx,'YTick',[]);
end
if ~isfield(guiData,'rasterLine') || ~isgraphics(guiData.rasterLine)
    guiData.rasterLine = plot(guiData.rasterAx, NaN, NaN, '.k');
end
...
theseISIclean = theseISI(theseISI >= param.tauC);
...
theseSpikeTimesAmp = ephysData.spike_times_timeline(ephysData.spike_templates == thisUnit);

% ??? scatter
if isfield(guiData,'tempAmpli') && isgraphics(guiData.tempAmpli), delete(guiData.tempAmpli); end
if isfield(guiData,'currTempAmpli') && isgraphics(guiData.currTempAmpli), delete(guiData.currTempAmpli); end
if isfield(guiData,'rpvAmpli') && isgraphics(guiData.rpvAmpli), delete(guiData.rpvAmpli); end

axes(guiData.ampliAx); yyaxis left; hold on;

guiData.tempAmpli = scatter(theseSpikeTimesAmp, theseAmplis, 10, 'black', 'filled', ...
    'ButtonDownFcn', @ampli_click);

theseISI2      = diff(theseSpikeTimesAmp);
theseISIclean2 = theseISI2(theseISI2 >= param.tauC);
off2           = find(theseISIclean2 < (2/1000)); 
rpvIndices     = off2;
rpvTimes       = theseSpikeTimesAmp(rpvIndices);
rpvAmps        = theseAmplis(rpvIndices);
guiData.rpvAmpli = scatter(rpvTimes, rpvAmps, 10, 'magenta', 'filled', ...
    'ButtonDownFcn', @ampli_click);

if ~isempty(theseSpikeTimesAmp)
    centerT   = theseSpikeTimesAmp(min(max(iChunk,1), numel(theseSpikeTimesAmp)));
    currTimes = theseSpikeTimesAmp(theseSpikeTimesAmp >= centerT-0.1 & theseSpikeTimesAmp <= centerT+0.1);
    currAmps  = theseAmplis(theseSpikeTimesAmp >= centerT-0.1 & theseSpikeTimesAmp <= centerT+0.1);
else
    currTimes = [];
    currAmps  = [];
end
guiData.currTempAmpli = scatter(currTimes, currAmps, 10, 'blue', 'filled', ...
    'ButtonDownFcn', @ampli_click);

set(guiData.ampliAx.YAxis(1), 'Limits', [0, max(1, round(max(theseAmplis)))])

axes(guiData.ampliAx); yyaxis right;
binSize  = 20;
timeBins = 0:binSize:ceil(ephysData.spike_times_timeline(end));
while length(timeBins)==1    
    binSize  = binSize/2;
    timeBins = 0:binSize:ceil(ephysData.spike_times_timeline(end));
end
[n, x] = hist(theseSpikeTimesAmp, timeBins);
n = n ./ binSize;

set(guiData.spikeFR, 'XData', x, 'YData', n);
set(guiData.ampliAx.YAxis(2), 'Limits', [0, 2 * max(1, ceil(max(n)))])
...
%% 10) raster plot
if isempty(theseSpikeTimesAmp)
    set(guiData.rasterLine, 'XData', NaN, 'YData', NaN);
else
    set(guiData.rasterLine, 'XData', theseSpikeTimesAmp, 'YData', ones(size(theseSpikeTimesAmp)));
    xlim(guiData.rasterAx, [0, max(theseSpikeTimesAmp)]);
end
...
if plotRaw
    ...
    %% ?? amplitude axes/title/legend/spikeFR ??
    if ~isfield(guiData,'ampliAx') || ~isgraphics(guiData.ampliAx) || ...
            ~strcmp(get(guiData.ampliAx,'Type'),'axes')
        guiData.ampliAx = subplot(6,13,[67:70,73:76]);
        hold(guiData.ampliAx,'on');
    ...
    theseISI      = diff(theseSpikeTimesAmp);
    theseISIclean = theseISI(theseISI >= param.tauC);
    theseOffendingSpikes = find(theseISIclean < (2/1000)); 
    rpvIndices = theseOffendingSpikes;
    rpvTimes   = theseSpikeTimesAmp(rpvIndices);
    rpvAmps    = theseAmplis(rpvIndices);
    guiData.rpvAmpli = scatter(rpvTimes, rpvAmps, 10, 'magenta', 'filled', ...
        'ButtonDownFcn', @ampli_click);
    ...
    set(guiData.spikeFR, 'XData', x, 'YData', n);
    set(guiData.ampliAx.YAxis(2), 'Limits', [0, 2 * max(1, round(max(n)))])
