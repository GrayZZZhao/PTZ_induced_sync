...
if param.plotGlobal
    ...
    subplot(231)
    title('Somatic unit waveforms');hold on;
    ...
    title('non-somatic unit waveforms');hold on;
    ...
    title('Noise unit waveforms');hold on;
    ...
    rawSingleUnitLines = arrayfun(@(x) plot(squeeze(qMetric.rawWaveforms(singleUidx(x)).spkMapMean(qMetric.rawWaveforms(singleUidx(x)).peakChan,:)), 'linewidth', 1, 'Color', 'k'), ...
         ...
         % ==== Extract all raw single-unit waveforms used in subplot(234) ====
    % singleUidx 已经在上面定义，这里直接使用
    singleUidx = find(unitType == 1);          % 单位索引
    nSingle    = numel(singleUidx);

    % 用第一条 single unit 的波形确定时间点个数
    firstUnit    = singleUidx(1);
    firstPeakCh  = qMetric.rawWaveforms(firstUnit).peakChan;
    nSamples     = size(qMetric.rawWaveforms(firstUnit).spkMapMean, 2);

    % 预分配 [nSingle x nSamples] 的 double 矩阵
    TypeOneRawWaveForm = nan(nSingle, nSamples);

    for ii = 1:nSingle
        thisUnit   = singleUidx(ii);
        thisPeakCh = qMetric.rawWaveforms(thisUnit).peakChan;

        thisWave   = qMetric.rawWaveforms(thisUnit).spkMapMean(thisPeakCh, :);
        TypeOneRawWaveForm(ii, :) = double(thisWave(:)).';   % 每行一个 waveform
    end
    % ===========================================
