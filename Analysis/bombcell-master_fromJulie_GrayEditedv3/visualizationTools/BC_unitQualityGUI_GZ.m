...
function unitQualityGuiHandle = BC_unitQualityGUI_GZ(memMapData, ephysData, qMetric, param, probeLocation, unitType, plotRaw)
...
%% key press callback
    function KeyPressCb(~, evnt)
        ...
        elseif strcmpi(evnt.Key, 'u') % jump to particular unit
...
t = int32(firstSpike * ephysData.ephys_sample_rate): ...
...
for iChanToPlot = 1:length(chansToPlot)
    ...
    if ~isempty(theseTimesCenter)
        for iTimes = 1:size(theseTimesCenter, 1)
            if exist('theseTimesFull','var') && ~any(mod(theseTimesFull(iTimes, :), 1))
                set(rawSpikeLines(iChanToPlot), ...
                    'XData', theseTimesFull(iTimes, :), ...
                    'YData', thisMemMap(iChanToPlot, int32(theseTimesFull(iTimes, :))-t(1)));
            end
        end
    end
end
