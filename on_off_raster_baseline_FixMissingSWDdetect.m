...
spike_time_wt = readNPY('F:\YlabPTZnxpxlrecording\2025\2025-04-23_517_WT-female-adult\2025-04-23_16-31-14\Record Node 101\experiment1\recording2\continuous\Neuropix-PXI-100.ProbeA-AP\spike_times.npy');
spike_time_full_wt = double(spike_time_wt) / 30000;
spike_templates_wt = readNPY('F:\YlabPTZnxpxlrecording\2025\2025-04-23_517_WT-female-adult\2025-04-23_16-31-14\Record Node 101\experiment1\recording2\continuous\Neuropix-PXI-100.ProbeA-AP\spike_templates.npy');
...
[swd_events_filtered, keep_idx] = curate_swd_events(corrected_baseline, 2500, swd_events, 10, timepoint_array, neuron_id, struct('max_neurons',600));
...
function [swd_events_filtered, keep_idx] = curate_swd_events( ...
    ...
    S.win_half    = max(0.5, win_sec/2);   % 半宽；总视野=2*win_half
    S.fs          = fs;
    S.lfp         = lfp_data;
    ...
    % ---- Figure：使用 WindowKeyPressFcn 捕获全窗口按键 ----
    S.fig = figure('Name','SWD curation','Color','w', ...
        'KeyPressFcn',       @(h,e) keyHandler(e), ...        % 兼容旧版
        'WindowKeyPressFcn', @(h,e) keyHandler(e), ...        % 关键：全窗口键盘回调
        'NumberTitle','off','Units','normalized','Position',[0.08 0.08 0.84 0.78]);

    % 上：LFP；下：Raster（不要再给 axes 设置 KeyPressFcn）
    S.ax1 = axes('Parent',S.fig,'Position',[0.07 0.36 0.90 0.55]);
    S.ax2 = axes('Parent',S.fig,'Position',[0.07 0.18 0.90 0.14]);
    if ~S.haveRaster, set(S.ax2,'Visible','off'); end

    % 底部按钮（给每个按钮加 KeyPressFcn 转发）
    uicontrol(S.fig,'Style','pushbutton','String','<< 上一个 (A)',...
        'Units','normalized','Position',[0.07 0.06 0.11 0.07],...
        'Callback',@(h,~) prevEv(), 'KeyPressFcn',@(h,e) keyHandler(e));
    uicontrol(S.fig,'Style','pushbutton','String','下一个 (D) >>',...
        'Units','normalized','Position',[0.19 0.06 0.11 0.07],...
        'Callback',@(h,~) nextEv(), 'KeyPressFcn',@(h,e) keyHandler(e));
    uicontrol(S.fig,'Style','pushbutton','String','保留 (K/Y)',...
        'Units','normalized','Position',[0.31 0.06 0.10 0.07],...
        'Callback',@(h,~) keepThis(true), 'KeyPressFcn',@(h,e) keyHandler(e));
    uicontrol(S.fig,'Style','pushbutton','String','剔除 (X/R)',...
        'Units','normalized','Position',[0.42 0.06 0.10 0.07],...
        'Callback',@(h,~) keepThis(false), 'KeyPressFcn',@(h,e) keyHandler(e));
    uicontrol(S.fig,'Style','pushbutton','String','切换 (T)',...
        'Units','normalized','Position',[0.53 0.06 0.08 0.07],...
        'Callback',@(h,~) toggleKeep(), 'KeyPressFcn',@(h,e) keyHandler(e));
    uicontrol(S.fig,'Style','pushbutton','String','宽窗 +1s (W)',...
        'Units','normalized','Position',[0.62 0.06 0.09 0.07],...
        'Callback',@(h,~) widenWin(), 'KeyPressFcn',@(h,e) keyHandler(e));
    uicontrol(S.fig,'Style','pushbutton','String','窄窗 -1s (S)',...
        'Units','normalized','Position',[0.72 0.06 0.09 0.07],...
        'Callback',@(h,~) narrowWin(), 'KeyPressFcn',@(h,e) keyHandler(e));
    uicontrol(S.fig,'Style','pushbutton','String','全局/局部 (V)',...
        'Units','normalized','Position',[0.82 0.06 0.10 0.07],...
        'Callback',@(h,~) toggleGlobal(), 'KeyPressFcn',@(h,e) keyHandler(e));
    if S.haveRaster
        uicontrol(S.fig,'Style','pushbutton','String','显示/隐藏 Raster (H)',...
            'Units','normalized','Position',[0.07 0.005 0.20 0.05],...
            'Callback',@(h,~) toggleRaster(), 'KeyPressFcn',@(h,e) keyHandler(e));
    end
    ...
    function keyHandler(e)
        ...
        switch lower(e.Key)
            ...
            case {'x','r','delete'},     keepThis(false);
            case {'t'},                  toggleKeep();
            case {'w'},                  widenWin();
            case {'s'},                  narrowWin();
            case {'v'},                  toggleGlobal();
            ...
            case {'return','enter'}
                if ishandle(S.fig), uiresume(S.fig); close(S.fig); end
            case {'escape','q'}
